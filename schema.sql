...
CREATE INDEX idx_import_log_status_imported_at
ON data.import_log (status, imported_at DESC);

CREATE INDEX idx_import_log_line_status
ON data.import_log (line, status);

CREATE TABLE data.did_query (
    line VARCHAR(10) NOT NULL,
    csv_date CHAR(10) NOT NULL,
    did TEXT,
    gcode TEXT
);

CREATE INDEX idx_did_query_main_query
ON data.did_query (gcode, csv_date, line)
INCLUDE (did);
